...
description: "Who's actually trained on what — a skill × person grid flagging any skill where nobody can work it unsupervised yet, so a single absence doesn't stop the work. I is in training, L has learned it, U can work it unsupervised, O can train others. Use when: A process depends on more than one person being able to do it — you want to know honestly where that's actually true and where it isn't. You want to catch a single-point-of-failure skill before it becomes a problem, not after someone's out sick. You've just written a work instruction and want to track who's actually been trained on it. Not for: Only one person will ever do this task — a training matrix for a team of one adds process without adding redundancy. You don't have real information about who can do what — a matrix filled with guesses is worse than no matrix, since it looks like coverage that isn't real. The skill list is still unsettled — write the work instruction first, then track training against it."
---
...
# Training matrix with I, L, U, O skill levels (ILUO)

Who's actually trained on what — a skill × person grid flagging any skill where nobody can work it unsupervised yet, so a single absence doesn't stop the work. I is in training, L has learned it, U can work it unsupervised, O can train others.
...
Training matrix with I, L, U, O skill levels (ILUO) is licensed [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Attribution: Katafacts (katafacts.com).
