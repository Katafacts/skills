---
name: makigami
description: "Zoom into one office or service process to see why it's slow or error-prone: follow a real case through every person, handoff, document and system, with lead time, handoffs, re-keying and first-pass yield computed. Use it when a value stream map points at a problem process, or for any process full of approvals and rework. Use when: An office, service or knowledge-work process crosses several people or departments and takes far longer than the work itself should. A value stream map shows one process box with long waits or poor quality, and nobody can say exactly why. Not for: You want an overview of a whole value stream across departments — start with a value stream map, then zoom into the worst box with a Makigami. The work stays with one person from start to finish — a work instruction or a simple time study fits better. You can only describe how the process is supposed to work. Go and follow a real case first; a Makigami of the official procedure hides exactly the waits and workarounds you're looking for."
---
...
- **map** — Work in layers. First who does what: a lane for everyone who touches the work, and the steps of the real case in order, each named verb + noun. Mark a step confirmed once someone from that lane has checked it against the case. Working as a team: save the map, share it, and ask someone from each lane to confirm that lane. They see the whole map but can only confirm or correct their own steps, and say what was wrong; the map records who confirmed each step and when. Then how long: the hands-on work time, and how long it waited before the step started — counting nights, weekends and waiting for meetings. Waits and hands-on work each have their own unit, so waits can run in days while work stays in minutes; work is converted before it's added to lead time. Leave a time blank rather than guess. Note what you saw at each step — the email, form or ticket — so the map can be checked against the evidence. Then what it touches: every document, system and channel, and whether data already captured somewhere is typed in again.
