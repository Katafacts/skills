---
name: ctq-tree
description: "Translates a customer need into specific, measurable requirements — need, then the drivers that make it satisfied, then a concrete measurable target for each — with any need still missing a real specification named plainly. Use when: You have real customer needs — ideally traced to a VOC evidence log or affinity diagram — and want to translate them into something a team can design, build, and measure against. You want to know exactly which needs are actually specified and which are still just good intentions with no measurable target. Not for: You don't have a real customer need yet, just a feature idea — start with VOC evidence, not a CTQ tree, or you'll be specifying something nobody asked for. You want the raw synthesis of quotes into themes — that's the VOC evidence log or affinity diagram; a CTQ tree assumes the need is already named."
---
...
Critical-to-quality (CTQ) tree is licensed [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Attribution: Katafacts (katafacts.com).
