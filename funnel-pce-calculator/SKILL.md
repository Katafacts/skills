...
description: "The single flow-efficiency number for a sales pipeline — process time as a share of total time in the funnel — from a short list of stages, no full value stream map required. Use when: You want the headline flow-efficiency number fast — a sanity check before deciding whether a full funnel value stream map is worth the time. You already have per-stage process and wait time from your customer relationship management (CRM) system and just want it turned into a defensible percentage with the arithmetic shown. Not for: You need to know *which* stage to fix, not just the overall number — that's the sales funnel value stream map or the bottleneck analysis, both of which compute this same number as part of a fuller picture. You want waste findings, a Pareto ranking, or countermeasures — this calculator deliberately doesn't ask for the observation notes those are drawn from."
---
...
# Funnel flow efficiency calculator
...
- You already have per-stage process and wait time from your customer relationship management (CRM) system and just want it turned into a defensible percentage with the arithmetic shown.
...
- **Mixing deal segments with genuinely different natural cycles to get one 'overall' number.** — A blended self-serve-plus-enterprise flow efficiency describes neither segment — same scope discipline as every other value-stream kata in this catalogue.
...
Funnel flow efficiency calculator is licensed [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Attribution: Katafacts (katafacts.com).
