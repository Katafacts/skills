...
description: "How much credit for an outcome shifts depending purely on which honest model you pick — first-attributed, last-attributed, or an even split — computed across every real contributor, with the ones most sensitive to model choice flagged plainly rather than settled by whichever one looks most convenient. Use when: Multiple changes or contributors could plausibly be credited for one observed outcome, and nothing about the timing alone proves which one actually caused it. You want to know honestly how much your current crediting convention would change if you picked a different, equally reasonable model — before betting a bigger decision on it. Not for: You already ran a real controlled experiment (a holdout, an A/B test) that isolated the actual cause — trust that over any of this tool's three models. There's genuinely only one contributor in play — there's no real attribution question to reality-check."
---
